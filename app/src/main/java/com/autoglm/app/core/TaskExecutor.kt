...
class TaskExecutor(
    ...
    private suspend fun executeLoop(task: String) {
        ...
        withContext(Dispatchers.Main) { accessibilityService.performHome() }
        delay(1000) // 等待动画完成
        ...
        while (stepCount < maxSteps && _status.value is ExecutionStatus.Running) {
            ...
            delay(30) // 短暂延迟确保悬浮窗已隐藏

            val screenshot = takeScreenshot()

            // 立即恢复悬浮窗显示
            onShowFloatingWindow?.invoke()
            onStatusUpdate?.invoke("步骤 $stepCount: 分析中", "")
            ...
            val response =
                    try {
                        ...
                        val result =
                                aiClient.analyzeScreenAndPlan(screenshot, task, executedActions)
                        ...
                        if (aiClient.isFatalError(e)) {
                            ...
                            addLog(
                                    LogType.ERROR,
                                    "⚠️ 检测到致命错误 ($consecutiveFatalErrors/3): ${e.message?.take(50)}"
                            )
                        ...
                        if (!errorMessage.contains("API 配置") && !errorMessage.contains("设置中添加")) {
                            val errorFeedback = "【上次操作失败】${errorMessage.take(50)}，请换一种方式尝试"
            ...
            // 输出原始响应以便调试
            response.rawResponse?.let { addLog(LogType.INFO, "AI 原始响应: $it") }
            ...
            for ((index, actionItem) in allActions.withIndex()) {
                ...
                if (resultMsg.isEmpty()) {
                    ...
                    val failureMsg =
                            if (actionItem is Action.Launch) {
                                "【Launch失败】$resultMsg"
                            } else {
                                "【操作失败】$resultMsg"
                            }
        ...
        if (stepCount >= maxSteps) {
            addLog(LogType.WARNING, "⚠️ 达到最大步数限制 ($maxSteps)")
        }
    }
    ...
    private suspend fun executeActionWithHide(action: Action): String {
        ...
        val needHide = action is Action.Tap || action is Action.Swipe || action is Action.LongPress

        return withContext(Dispatchers.Main) {
            when (action) {
                is Action.Tap -> {
                    addLog(LogType.INFO, "执行: 点击 (${action.x}, ${action.y})")
                    if (needHide) onHideFloatingWindow?.invoke()
                    val result =
                            if (accessibilityService.performTap(action.x, action.y)) ""
                            else "Tap执行失败"
                    if (needHide) onShowFloatingWindow?.invoke()
                    result
                }
                is Action.Swipe -> {
                    addLog(
                            LogType.INFO,
                            "执行: 滑动 (${action.x1},${action.y1})→(${action.x2},${action.y2})"
                    )
                    if (needHide) onHideFloatingWindow?.invoke()
                    val result =
                ...
                is Action.LongPress -> {
                    ...
                    if (needHide) onHideFloatingWindow?.invoke()
                    val result =
                            if (accessibilityService.performLongPress(
                                            action.x,
                                            action.y,
                                            action.duration
                                    )
    ...
    private fun launchApp(appName: String): String {
        return try {
            ...
            if (fuzzyMatches.isNotEmpty()) {
                ...
                val suggestions =
                        fuzzyMatches
                                .sortedBy { Math.abs(it.length - appName.length) }
                                .take(3)
                                .joinToString(", ")
