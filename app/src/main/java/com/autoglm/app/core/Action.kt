...
sealed class Action {
    ...
    /** 滑动操作 */
    data class Swipe(val x1: Int, val y1: Int, val x2: Int, val y2: Int, val duration: Int = 300) :
            Action() {
    ...
    /** 请求用户介入 用于敏感操作(支付/删除/确认)或 AI 无法继续时 */
    data class AskUser(val reason: String, val suggestion: String = "") : Action() {
        override fun toString() = "请求用户介入: $reason"
    }

    /** 启动应用操作 直接通过系统启动应用，不需要模拟点击图标 */
    data class Launch(val appName: String) : Action() {
        override fun toString() = "启动应用: $appName"
    }

    /** 确认/回车操作 模拟按下输入法的确认键或回车键 */
    object Enter : Action() {
    ...
    /** 基于标记编号点击 (Set-of-Marks) 比坐标点击更精准 */
