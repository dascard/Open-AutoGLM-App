...
object FileLogger {
    ...
    private var currentLogLevel = LEVEL_INFO // 默认 INFO
    ...
    fun setLogLevel(level: Int) {
        currentLogLevel = level.coerceIn(LEVEL_OFF, LEVEL_DEBUG)
    }
    ...
    private fun cleanOldLogs(logDir: File) {
        try {
            val logFiles =
                    logDir.listFiles { file -> file.name.startsWith("log_") }?.sortedByDescending {
                        it.lastModified()
                    }
                            ?: return
    ...
    fun e(tag: String, message: String, throwable: Throwable? = null) {
        ...
        throwable?.let {
            ...
            it.stackTrace.take(10).forEach { element -> writeToFile("E", tag, "    at $element") }
        }
    }
    ...
    private fun rotateLogFile() {
        try {
            ...
            logFile?.let { file -> printWriter = PrintWriter(FileWriter(file, false), true) }
        } catch (e: Exception) {
